...
enum {
	...
	DESKTOP_ENTRY_SHOW_IN_UKUI   = 1 << 2,
...
static guint get_flags_from_key_file(DesktopEntry* entry, GKeyFile* key_file, const char* desktop_entry_group)
{
  ...
  if (show_in_ukui)
    flags |= DESKTOP_ENTRY_SHOW_IN_UKUI;
  if (tryexec_failed)
...
#define GET_LOCALE_STRING(n) g_key_file_get_locale_string (key_file, desktop_entry_group, (n), NULL, NULL)
  ...
  retval->full_name    = GET_LOCALE_STRING ("X-UKUI-FullName");
...
#undef GET_LOCALE_STRING
  ...
  menu_verbose ("Desktop entry \"%s\" (%s, %s, %s, %s, %s) flags: NoDisplay=%s, Hidden=%s, ShowInUKUI=%s, TryExecFailed=%s\n",
                retval->basename,
                retval->name,
                ...
                retval->flags & DESKTOP_ENTRY_SHOW_IN_UKUI  ? "(true)" : "(false)",
...
{
	return (entry->flags & DESKTOP_ENTRY_SHOW_IN_UKUI) != 0;
}
