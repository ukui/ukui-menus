...
static MenuMonitorNotify* ukui_menu_monitor_notify_ref(MenuMonitorNotify* notify);
static void ukui_menu_monitor_notify_unref(MenuMonitorNotify* notify);
...
static void invoke_notifies(MenuMonitor* monitor, MenuMonitorEvent  event, const char* path)
{
  ...
  g_slist_foreach (copy,
		   (GFunc) ukui_menu_monitor_notify_ref,
		   NULL);
  ...
  while (tmp != NULL)
      ...
      if (notify->notify_func)
	{
      ...
      ukui_menu_monitor_notify_unref(notify);
  ...
  while (tmp != NULL)
    {
      ...
      ukui_menu_monitor_ref(event_info->monitor);
...
static MenuMonitor* lookup_monitor(const char* path, gboolean is_directory)
{
  ...
  if (retval == NULL)
  ...
  else
    {
      ...
      return ukui_menu_monitor_ref(retval);
    }
}

MenuMonitor* ukui_menu_monitor_file_get(const char* path)
{
...
MenuMonitor* ukui_menu_monitor_ref(MenuMonitor* monitor)
{
...
void menu_monitor_unref(MenuMonitor* monitor)
{
  ...
  g_slist_foreach (monitor->notifies, (GFunc) ukui_menu_monitor_notify_unref, NULL);
...
void ukui_menu_monitor_notify_remove(MenuMonitor* monitor, MenuMonitorNotifyFunc notify_func, gpointer user_data)
{
	GSList* tmp = monitor->notifies;
